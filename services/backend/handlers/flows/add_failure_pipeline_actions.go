...
func AddFlowFailurePipelineActions(context *gin.Context, db *bun.DB) {
	...
	// Audit
	err = functions_project.CreateAuditEntry(flowDB.ProjectID, "create", "Action added to Flow Failure Pipeline", db, context)
	if err != nil {
		log.Error(err)
	}
