...
func UpdateProject(context *gin.Context, db *bun.DB) {
	...
	_, err = db.NewUpdate().Model(&project).Column("name", "description", "shared_runners", "icon", "color", "enable_auto_runners", "disable_runner_join", "predefined_flow_actions").Where("id = ?", projectID).Exec(context)
	if err != nil {
