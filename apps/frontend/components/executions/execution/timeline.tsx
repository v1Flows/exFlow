...
export default function ExecutionTimeline({
  ...
  async function interactStep(step: any, status: boolean) {
    ...
    await InteractExecutionStep(execution.id, step.id, step)
      ...
      .catch((err) => {
        toast.danger("Interaction failed", { description: err.message });
      });
  }
  ...
  return (
    <div className="flex h-full flex-col gap-6 overflow-hidden lg:flex-row">
      ...
      <Card className="h-full w-full border border-white/10 bg-surface/60 backdrop-blur-md lg:w-1/3">
        <Card.Header className="sticky top-0 z-10 flex items-center justify-between border-b border-separator bg-surface/50 px-4 py-3 backdrop-blur-md">
          <span className="font-semibold text-foreground">Execution Steps</span>
          <Chip color="accent" size="sm" variant="soft">
            <Chip.Label>{steps.length} Steps</Chip.Label>
          </Chip>
        </Card.Header>
        ...
        <ScrollShadow className="flex-1 p-4">
          <div ref={listRef} className="relative flex flex-col">
            {/* Vertical Line */}
            <div className="absolute bottom-4 left-[19px] top-4 z-0 w-[2px] bg-default/50" />
            ...
            {steps.map((step: any, index: number) => {
              ...
              return (
                ...
                >
                  {/* Timeline Dot */}
                  <div
                    className={`absolute left-[10px] top-4 flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded-full border-2 bg-background transition-colors ${isSelected ? "border-accent" : "border-default"}`}
                  >
                    <div
                    ...
                    <Card
                      className={`w-full border transition-all ${
                        isSelected
                          ...
                          : "border-default/50 bg-surface/40 hover:bg-surface/60"
                      }`}
                    >
                          ...
                          <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="flex flex-col gap-0.5 overflow-hidden">
                              <div className="flex items-center gap-2">
                                <span
                                  className={`truncate font-medium ${isSelected ? "text-accent" : "text-foreground"}`}
                                >
      ...
      <Card className="h-full w-full border border-white/10 bg-surface/60 backdrop-blur-md lg:w-2/3">
        {selectedStep ? (
          <>
            <Card.Header className="flex flex-col gap-4 border-b border-separator bg-surface/50 px-6 py-4 backdrop-blur-md">
              <div className="flex w-full items-start justify-between">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="text-sm flex items-center gap-4 text-muted">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    className={autoScrollEnabled ? "text-accent" : "text-muted"}
                    variant="ghost"
              ...
              {selectedStep.status === "interactionWaiting" && (
                <div className="rounded-md flex w-full items-center gap-4 border border-warning/20 bg-warning/10 p-4">
            ...
            <div className="relative flex flex-1 flex-col overflow-hidden">
              ...
              >
                {selectedStep.messages && selectedStep.messages.length > 0 ? (
                  <div className="flex flex-col gap-0.5">
                    {selectedStep.messages
                      ...
                      .map((line: any, i: number) => (
                        ...
                        >
                          <span className="shrink-0 select-none text-muted opacity-50 text-xs pt-0.5">
                            {line.timestamp
                  ...
                  <div className="flex h-full flex-col items-center justify-center gap-2 text-muted opacity-50">
          ...
          <div className="flex h-full flex-col items-center justify-center gap-4 text-muted">
            <div className="flex size-20 items-center justify-center rounded-full bg-surface-secondary/50">
              <Icon
